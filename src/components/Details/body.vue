<template lang="html">
  <div class="col-xs-8 col-xs-offset-2">
    ...
    <div class="comments_area">
      <mu-list>
        ...
        <mu-list-item v-for="(item, index) in commentsList"  :key="item.commerId._id"  :title="item.commerId.nickname">
          ...
          <mu-text-field :id="'field_'+item._id" class="noShow" hintText="回复些什么好呢" @keyup.native.enter="reply(item._id, item.commerId._id)" v-model="reply_words" fullWidth  multiLine :rows="3" :rowsMax="6"/>
    ...
    <div class="input_area">
      <mu-text-field v-model="input_words" :disabled="(getUserInfoSession._id === 0)" @keyup.native.enter="comments" label="留下你的脚印~~" fullWidth icon=":fa fa-commenting-o"  multiLine :rows="6" labelFloat/>
